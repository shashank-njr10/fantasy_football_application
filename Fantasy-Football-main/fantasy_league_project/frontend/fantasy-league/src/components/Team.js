...
const Team = ({
  ...
  let defendersList = trueUser.players.filter(
    (player) => player.position === 2
  );
  let midfieldersList = trueUser.players.filter(
  ...
  const keepersComponent = keepersList.map((keeper) => (
    <PlayerCard
      userplayer={keeper}
  ...
  const defendersComponent = defendersList.map((defender) => (
    <PlayerCard
      userplayer={defender}
      playersList={playersList}
      removePlayer={removePlayer}
  ...
  const midfieldersComponent = midfieldersList.map((midfielder) => (
    <PlayerCard
      userplayer={midfielder}
      playersList={playersList}
      removePlayer={removePlayer}
      key={midfielder.id}
      trueUser={trueUser}
  ...
  const strikersComponent = strikersList.map((striker) => (
    <PlayerCard
      userplayer={striker}
      playersList={playersList}
      removePlayer={removePlayer}
  ...
  return (
    <section class="team-section">
      <div id="user-display">
        ...
        <h3>Gameweek Points: {userGWScore(trueUser)}</h3>
        <h3>Total Points: {userOverallScore(trueUser)}</h3>
      </div>
